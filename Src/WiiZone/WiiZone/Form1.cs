        ...
        public void taskM()
        {
            for (; ; )
            {
                if (Getstate)
                {
                    ...
                    mousex = (Math.Pow(irx > 0 ? irx : -irx, 3.5f) / Math.Pow(1200f, 2.5f) + 1.5f) * (irx > 0 ? 1f : -1f);
                    mousey = (Math.Pow(iry > 0 ? iry : -iry, 2f) / Math.Pow(1200f, 1f) + 1.5f) * (iry > 0 ? 1f : -1f);
