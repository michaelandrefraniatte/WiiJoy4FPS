        ...
        public void taskM()
        {
            for (; ; )
            {
                if (Getstate)
                {
                    ...
                    mousex = (Math.Pow(irx > 0 ? irx : -irx, 3.5f) / Math.Pow(1200f, 2.5f) + 3.99f) * (irx > 0 ? 1f : -1f);
                    mousey = (Math.Pow(iry > 0 ? iry : -iry, 2f) / Math.Pow(1200f, 1f) + 3.99f) * (iry > 0 ? 1f : -1f);
                    slowing += 1f;
                    if (slowing >= 3f)
                    {
                        MouseBrink((int)(-mousex * 0.25f), (int)(mousey * 0.25f));
                        slowing = 0f;
                    }
        ...
        public void taskK()
        {
            for (; ; )
            {
                ...
                if (Getstate)
                {
                    ...
                    valchanged(32, mWSNunchuckStateRawValuesX > -41);
                    if (wd[32] == 1)
                    {
                        SimulateKeyUp(VK_P, VK_P);
                        SimulateKeyUp(VK_NUMPAD8, S_NUMPAD8);
                        SimulateKeyUp(VK_NUMPAD4, S_NUMPAD4);
                        SimulateKeyUp(VK_NUMPAD2, S_NUMPAD2);
                        SimulateKeyUp(VK_NUMPAD6, S_NUMPAD6);
                    ...
                    if (mWSNunchuckStateRawValuesX > -41)
                    {
                    ...
                    {
                        valchanged(12, mWSButtonStateHome);
                        if (wd[12] == 1)
                            SimulateKeyDown(VK_P, S_P);
                        if (wu[12] == 1)
                            SimulateKeyUp(VK_P, S_P);
                        valchanged(28, mWSButtonStateUp);
                        if (wd[28] == 1)
                            SimulateKeyDown(VK_NUMPAD8, S_NUMPAD8);
                        if (wu[28] == 1)
                            SimulateKeyUp(VK_NUMPAD8, S_NUMPAD8);
                        valchanged(31, mWSButtonStateLeft);
                        if (wd[31] == 1)
                            SimulateKeyDown(VK_NUMPAD4, S_NUMPAD4);
                        if (wu[31] == 1)
                            SimulateKeyUp(VK_NUMPAD4, S_NUMPAD4);
                        valchanged(29, mWSButtonStateDown);
                        if (wd[29] == 1)
                            SimulateKeyDown(VK_NUMPAD2, S_NUMPAD2);
                        if (wu[29] == 1)
                            SimulateKeyUp(VK_NUMPAD2, S_NUMPAD2);
                        valchanged(30, mWSButtonStateRight);
                        if (wd[30] == 1)
                            SimulateKeyDown(VK_NUMPAD6, S_NUMPAD6);
                        if (wu[30] == 1)
                            SimulateKeyUp(VK_NUMPAD6, S_NUMPAD6);
                        valchanged(0, mWSButtonStateMinus);
                        if (wd[0] == 1)
                            SimulateKeyDown(VK_T, S_T);
                        if (wu[0] == 1)
                            SimulateKeyUp(VK_T, S_T);
                        valchanged(1, mWSButtonStatePlus);
                        if (wd[1] == 1)
