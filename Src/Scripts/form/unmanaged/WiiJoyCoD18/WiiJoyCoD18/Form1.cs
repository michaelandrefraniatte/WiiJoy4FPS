...
namespace WiiJoyCoD18
{
    public partial class Form1 : Form
    {
        ...
        private const double REGISTER_IR = 0x04b00030, REGISTER_EXTENSION_INIT_1 = 0x04a400f0, REGISTER_EXTENSION_INIT_2 = 0x04a400fb, REGISTER_EXTENSION_TYPE = 0x04a400fa, REGISTER_EXTENSION_CALIBRATION = 0x04a40020, REGISTER_MOTIONPLUS_INIT = 0x04a600fe, viewpower1x = 0f, viewpower2x = 1f, viewpower3x = 0f, viewpower1y = 0.25f, viewpower2y = 0.75f, viewpower3y = 0f, dzx = 2.0f, dzy = 2.0f, lowsensx = 1f, lowsensy = 1f, centery = 140f;
        ...
        private static bool mWSIR1found, mWSIR0found, mWSButtonStateA, mWSButtonStateB, mWSButtonStateMinus, mWSButtonStateHome, mWSButtonStatePlus, mWSButtonStateOne, mWSButtonStateTwo, mWSButtonStateUp, mWSButtonStateDown, mWSButtonStateLeft, mWSButtonStateRight, running;
        ...
        private static void taskX()
        {
            while (running)
            {
                ...
                down = ButtonDownPressed;
                left = ButtonLeftPressed;
                right = ButtonRightPressed;
                up = ButtonUpPressed;
                rightstick = ButtonMinus;
                leftstick = TriggerLeftPressed;
                A = ButtonShoulderLeftPressed;
                ...
                if (ThumbLeftDown)
                    leftsticky = -32767;
                if (!ThumbLeftDown & !ThumbLeftUp)
                    leftsticky = 0;
                if (ThumbLeftRight)
                    leftstickx = 32767;
                if (ThumbLeftLeft)
                    leftstickx = -32767;
                if (!ThumbLeftLeft & !ThumbLeftRight)
                    leftstickx = 0;
        ...
        private static void ScanLeft()
        {
            ...
            {
                joystickGuid[0] = deviceInstance.InstanceGuid;
            }
            if (joystickGuid[0] != Guid.Empty)
            {
                joystick[0] = new Joystick(directInput, joystickGuid[0]);
                joystick[0].Properties.BufferSize = 128;
                joystick[0].Acquire();
            }
        }
        ...
        {
            joystick[0].Poll();
            var datas = joystick[0].GetBufferedData();
            foreach (var state in datas)
            {
                ...
                if (state.Offset == JoystickOffset.Buttons8 & state.Value == 0)
                    ButtonMinus = false;
                if (state.Offset == JoystickOffset.PointOfViewControllers0 & (state.Value == 9000 | state.Value == 13500 | state.Value == 4500))
                    ThumbLeftDown = true;
                if (state.Offset == JoystickOffset.PointOfViewControllers0 & !(state.Value == 9000 | state.Value == 13500 | state.Value == 4500))
                    ThumbLeftDown = false;
                if (state.Offset == JoystickOffset.PointOfViewControllers0 & (state.Value == 27000 | state.Value == 22500 | state.Value == 31500))
                    ThumbLeftUp = true;
                if (state.Offset == JoystickOffset.PointOfViewControllers0 & !(state.Value == 27000 | state.Value == 22500 | state.Value == 31500))
                    ThumbLeftUp = false;
                if (state.Offset == JoystickOffset.PointOfViewControllers0 & (state.Value == 18000 | state.Value == 13500 | state.Value == 22500))
                    ThumbLeftLeft = true;
                if (state.Offset == JoystickOffset.PointOfViewControllers0 & !(state.Value == 18000 | state.Value == 13500 | state.Value == 22500))
                    ThumbLeftLeft = false;
                if (state.Offset == JoystickOffset.PointOfViewControllers0 & (state.Value == 0 | state.Value == 4500 | state.Value == 31500))
                    ThumbLeftRight = true;
                if (state.Offset == JoystickOffset.PointOfViewControllers0 & !(state.Value == 0 | state.Value == 4500 | state.Value == 31500))
